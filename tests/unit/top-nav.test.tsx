...
// Route harness (Spec 009): `usePathname()` drives `aria-current` and the home
// button always navigates to the clean `/form` via `router.push`.
...
describe("TopNav — brand home + lang/theme toggles (Spec 009 / a11y)", () => {
  ...
  it("marks the home button as the current page on /form", () => {
  ...
  it("navigates to the clean /form route when the brand mark is clicked", () => {
    navState.setPath("/reader");
    renderTopNav();

    fireEvent.click(screen.getByRole("button", { name: "Voltar ao início" }));
    expect(navState.push).toHaveBeenCalledWith("/form");
  });
