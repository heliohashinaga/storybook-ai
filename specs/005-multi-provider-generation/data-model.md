...
| `image` | `IllustrationGenerator` | sim | Provedor de ilustração (definido pelo prefixo de `IMAGE_MODEL`; `createOpenRouterIllustration` se `openrouter/...`, `createOpenCodeIllustration` se `opencode-go/...`) + `image-optimizer` (lazy sharp). |
| `rateLimiter` | `RateLimiter` | sim | Rate limit de geração por IP (chave = hash + salt rotativo, IP não retido em claro), default 10 req/60 s (`STORY_RATE_LIMIT_MAX_REQUESTS`/`STORY_RATE_LIMIT_WINDOW_MS`). TTS tem limite próprio (`TTS_RATE_LIMIT_*`, feature 004). |
