...
- Q: A narração de IA deve estar ativa por padrão ou requerer ativação pelo usuário na tela? → A: **Não há switch de ativação de usuário na tela.** A narração por voz de IA é controlada por **configuração/environment** via `NARRATION_TTS_ENABLED`. Quando habilitada por config, o controle de leitura usa a voz de IA; quando desabilitada (default seguro), usa a voz de sistema (Web Speech) ou o fallback/desabilitado da FR-006. Preserva a postura "nada sai do dispositivo" quando a IA está desligada por config, sem adicionar um toggle de usuário na UI.
- Q: Existe `NARRATION_TTS_ENABLED` e ela liga/desliga a IA para o usuário? → A: **Sim** — `NARRATION_TTS_ENABLED` (env/config, server only) controla o uso da narração por IA; **não** há preferência/toggle de usuário na sessão nem na tela para isso. O default é desabilitada (`false`), caso em que o controle usa voz de sistema.
...
1. **Given** uma história já gerada com 3+ cenas **e** `NARRATION_TTS_ENABLED=true` (config) **When** o responsável aciona o controle de leitura na cena atual **Then** a narração inicia via áudio de IA (voz natural), coberta por indicação de estado acessível (anuncia "lendo" e o retoma/parado via `aria-live`), sem exigir outra ação extra.
   - **Caso base (IA desativada por config)**: **Given** `NARRATION_TTS_ENABLED=false` **When** o responsável aciona o controle de leitura **Then** o app usa a voz de sistema (Web Speech); o texto é legível e não há custo/uso de IA, sem qualquer switch de usuário na tela.
...
- **SC-002**: A percepção de naturalidade da voz de IA é **avaliada como superior** à voz de sistema em um simples teste tipo A/B (participantes preferem a voz de IA em ≥80% dos pares), sem que isso exija qualificar "natural" subjetivamente (critério observável).
